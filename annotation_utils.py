...
#An agreement has:
    ...
    #HasSz agreement metrics (Cohen's Kappa and F1)
    #SzFreq agreement metrics (Cohen's kappa and F1 overlap)
    #SzType agreement metrics (Cohen's kappa and F1 overlap)
...
DSSA: Different Span Same Annotation - the highlighted spans were completely different from each other, but the annotations were the same (hasSz only)
DSDA: Different Span Different Annotation - the highlighted spans were completely different from each other, and the annotations were different (hasSz only) 
DS: Different Span - the highlighted spans were completely different from each other, and no annotation relationship can be determined (szFreq or szType)
"""
class Agreement():
    ...
    def __calc_DSXX_agreement(self):
        ...
        for doc in self.least_annotated_annotator.documents:
            ...
            for annotation_1 in self.least_annotated_annotator.annotations:
                ...
                #skip if wrong document or if there are overlaps, or if the layer isn't hasSz
                ...
                for annotation_2 in self.most_annotated_annotator.annotations:
                    ...
                    #skip if wrong document or if there are overlaps
                    if annotation_2.Document != doc or annotation_2.layer != 'HasSeizures':
                        continue
                        
                    #check if either of the two annotations have overlaps, and if that overap was with these annotators
